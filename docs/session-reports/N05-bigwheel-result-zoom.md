...
2026-09-23 사용자 요청으로 휠 아래의 총 베팅 합계를 최소·최대 총액 한도 옆으로 옮겼다. 최근 결과 위치와 휠 크기는 유지했다. 최소 창에서는 이 정보 행만 10px로 표시해 Lv6 최대 금액도 한 줄에 담는다. 타입 검사·패키징을 통과했다. `npx playwright test tests/e2e/bigwheel.spec.ts tests/e2e/bigwheel-zoom.spec.ts --grep 'BW-08|BW-10|BW-14' --output=/tmp/molsino-bigwheel-total-results`로 관련 3개 사례가 통과했다(실패·스킵 0). Lv6 최소 창 캡처에서 합계와 한도가 한 줄에 온전히 표시됨을 직접 확인했다.
