...
**목적:** Blackjack의 Electron 구현 계약을 정의한다.

**요약:** 프로세스·모듈 구조, 창과 입력, IPC, 게임 엔진, 저장, 패키징과 검증 기준을 다룬다. 기능과 카지노 규칙의 기준은 [제품 설계서](blackjack-design.md)다. 기존 macOS 네이티브 및 Windows 별도 포팅 설계를 대체한다.

**목차**

- [1. 확정한 기술 결정](#1-확정한-기술-결정)
- [2. 펫 레퍼런스와 적용 범위](#2-펫-레퍼런스와-적용-범위)
- [3. 프로세스와 모듈 구조](#3-프로세스와-모듈-구조)
- [4. 창 구현과 OS별 처리](#4-창-구현과-os별-처리)
- [5. 표시 상태와 IPC 계약](#5-표시-상태와-ipc-계약)
- [6. 게임 엔진 계약](#6-게임-엔진-계약)
- [7. 명령 직렬화와 저장](#7-명령-직렬화와-저장)
- [8. 화면 컴포넌트와 레이아웃](#8-화면-컴포넌트와-레이아웃)
- [9. Electron 실행 경계](#9-electron-실행-경계)
- [10. 프로젝트 구조와 패키징](#10-프로젝트-구조와-패키징)
- [11. 검증과 완료 판정](#11-검증과-완료-판정)
- [12. 완료 정의](#12-완료-정의)

버전: 2.2 · 작성일: 2026-09-15 · 갱신일: 2026-09-18
...
위치·크기·색상 모드·불투명도·접힘·클릭 통과는 실행 중 상태이며 앱 재실행 시 각각 기본 위치·280×180 DIP·기본 색상·65%·펼침·대화형으로 초기화한다. 이 전체 초기화 계약의 완료 검증은 S11과 함께 TOBE로 보류한다. 창을 숨겼다가 복원하거나 Renderer를 재로드할 때는 현재 실행 중 값을 유지한다. 게임에 영향을 주는 pendingBet·betStep과 진행 중 판·잔액은 Session이 기준이며 재실행 후 복원한다. 로컬 파일은 평문이므로 딜러 카드 은닉은 UI 경계이며 부정행위 방지는 범위 밖이다.
...
O-01~O-09와 O-11은 제품 검증 조건이고 O-10은 별도 실험이다. 단, O-06 전체와 O-08의 창 설정 전체 재실행 초기화 부분은 S11과 함께 TOBE로 보류한다. 게임 세션 복원 등 O-08의 나머지 조건은 유지한다. 특히 O-05 실패를 고정 크기 UI로 대체해 완료 처리하지 않는다. DevTools가 열린 투명 창은 문서상 제약이 있으므로 투명도 최종 판정은 DevTools를 닫은 패키지에서 한다.
...
P0~P3는 기술적 완료를 판정할 범주이며 순차 개발 지시가 아니다. 실제 세션 번호와 실행 순서는 [작업 세션 로드맵](work-session-roadmap.md)이 기준이다.

| 범주 | 완료 판정 대상 |
| --- | --- |
| P0 — 오버레이 창 | Electron/Forge 골격과 두 OS 빌드, 가짜 카드를 사용한 창·포커스·크기 조절 검증. macOS에서 먼저 검증한다. |
| P1 — 게임 코어 | 공통 TypeScript 게임 엔진, 카지노 규칙과 JSON fixture 검증. |
| P2 — 앱 통합 | Main GameStore·Preload IPC·React UI·저장 통합, 양쪽 OS에서 같은 게임 실행. |
| P3 — 제품화 | 장애 복구·접근성·DPI·성능·설치본 검증 및 서명. |
...
현재 구현과 미구현 항목은 [구현 현황](implementation-status.md), 세션별 작업 순서는 [작업 세션 로드맵](work-session-roadmap.md)을 참조한다.
