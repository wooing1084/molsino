# Overlay Blackjack 빌드와 배포

갱신일: 2026-09-16

Overlay Blackjack은 Electron 데스크톱 앱이다. 최종 사용자에게는 Docker 이미지가 아니라 macOS 앱 ZIP 또는 Windows 설치 파일/포터블 ZIP을 전달한다.

## 1. 지원 산출물

| 대상 | 산출물 | 사용 방법 |
| --- | --- | --- |
| macOS Intel + Apple Silicon | Universal `.app`을 담은 ZIP | 압축 해제 후 `Overlay Blackjack.app` 실행 |
| macOS Apple Silicon 전용 | arm64 `.app`을 담은 ZIP | 압축 해제 후 `Overlay Blackjack.app` 실행 |
| Windows x64 설치형 | `Overlay-Blackjack-Setup.exe` | 설치 프로그램 실행 |
| Windows x64 포터블 | 앱 폴더 전체를 담은 ZIP | 압축을 모두 해제하고 `overlay-blackjack.exe` 실행 |
...
```text
out/make/zip/darwin/universal/Overlay Blackjack-darwin-universal-<version>.zip
```
...
```text
out/make/zip/darwin/arm64/Overlay Blackjack-darwin-arm64-<version>.zip
```

ZIP을 풀고 `Overlay Blackjack.app`을 실행한다. 로컬 패키지 폴더를 직접 실행하려면 다음과 같이 연다.

```sh
open "out/Overlay Blackjack-darwin-universal/Overlay Blackjack.app"
```
...
```text
out/make/squirrel.windows/x64/Overlay-Blackjack-Setup.exe
out/make/zip/win32/x64/Overlay Blackjack-win32-x64-<version>.zip
```

사용자에게는 일반적으로 `Overlay-Blackjack-Setup.exe`를 전달한다. Squirrel 업데이트용 `RELEASES`와 `.nupkg`도 함께 생성되지만, 자동 업데이트 서버를 구성하기 전에는 직접 전달할 필요가 없다.
...
Windows 사용자는 ZIP을 전부 푼 뒤 폴더 안의 `overlay-blackjack.exe`를 실행한다. EXE 하나만 떼어 전달하면 필요한 리소스가 없어 실행되지 않는다.
...
- `overlay-blackjack-macos-universal`
- `overlay-blackjack-windows-x64`
...
3. Windows ZIP을 Windows PC에 풀고 `overlay-blackjack.exe`를 실행한다.
...
```sh
BLACKJACK_SMOKE_EXECUTABLE="out/Overlay Blackjack-darwin-universal/Overlay Blackjack.app/Contents/MacOS/overlay-blackjack" npm run smoke
```
