...
**요약:** 2026-09-18 소스 기준으로 투명 오버레이, 크기 조절, 트레이와 단축키, 표시 상태·불투명도, IPC 신뢰 경계와 빌드·검증 기반이 연결돼 있다. 메인 메뉴·공용 잔액·바카라는 새 설계 단계다. 기존 미진행 세션은 폐기했으며 미구현·미검증 사실은 아래에 구분한다. 블랙잭의 코어·화면·저장 상태는 [블랙잭 구현 현황](../games/blackjack/implementation-status.md)에서 관리한다.
...
| 항목 | 현재 경계와 다음 위치 |
| --- | --- |
| 공용 잔액·메인 메뉴 | 현재 없음. 기존 블랙잭 전용 GameStore·저장·API를 N02에서 공용으로 연결하고 N03에서 게임 선택을 제공한다. [신규 계약](technical-design.md#9-여러-게임과-공용-잔액의-신규-계약) |
| 바카라 | 제품·기술·E2E 설계만 있다. 구현은 N04~N05다. [바카라 설계](../games/baccarat/product-design.md) |
...
| Renderer 장애 | 현재 `render-process-gone`은 숨김·오류 기록만 한다. 새 창 재생성은 없다. 실제 영향은 N01에서 평가하며 구 S16 일정은 폐기했다. |
