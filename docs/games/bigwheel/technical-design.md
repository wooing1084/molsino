...
표시 방식은 [제품 설계 §3.1](product-design.md#31-결과-주변-세-칸-확대)을 따른다. 변경 책임은 Renderer의 게임 컴포넌트·스타일·표시 타임라인에 한정한다. 코어의 54칸 배열·규칙 ID·당첨 추첨·정산·v4 저장·공개 IPC 계약은 유지한다.
...
순수 표시 매핑은 모든 54개 인덱스·경계 순환·동일 심볼 연속을 검사한다. 패키지 검증 기준은 [E2E 설계](e2e-test-plan.md#3-세-칸-확대-검증)를 따른다.

`wheel-presentation.ts`의 `WheelMotion`은 순환 전의 연속 좌표 `wheelPosition`을 제공한다. 결과 대기 중에는 0.12칸/ms로 전진하며 결과 수신 시 현재 위치·속도를 잇는 3차 Hermite 곡선으로 감속한다. 정상 완료 기준은 시작 후 1,800ms이고, 감속 구간은 최소 1,350ms이므로 늦은 결과는 완료 시점을 연장한다. 목표는 결과 인덱스와 합동인 전방 좌표로 정해 역주행을 방지한다.

`bigwheel-window.tsx`는 앞뒤 여유 칸까지 렌더링하고 소수 좌표만큼 가로 이동시켜 세 칸 창으로 잘라 보여준다. 정지 후에도 연속 좌표를 유지해 위치 점프를 방지한다. 숨김·접힘에서는 프레임 예약을 중단한다.
