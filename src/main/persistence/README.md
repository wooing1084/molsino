...
**요약:** AtomicSessionRepository가 원자 파일 저장을, AppSessionRepository가 v2 검증·v1 이전을 맡는다. 기존 SessionRepository는 v1 형식 검증과 읽기에 사용한다.
...
- `app-session-repository.ts`: 공용 wallet·screen·게임 상태·잠금·revision의 v2 스키마, 새 파일 우선 로드, 기존 블랙잭 데이터 이전.
