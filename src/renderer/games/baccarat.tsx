...
const suits = { S: '♠', H: '♥', D: '♦', C: '♣' };
export function BaccaratGame({ state, balance, busy, saveError, internalError, error, runAction, onRetry, onMenu, overlayView }: {
  ...
  async function commitEdit() {
    ...
    const parsed = parseBetInput(draft, balance);
  ...
  const status = saveError ? '저장 실패 · 같은 판 저장을 재시도하세요' : internalError ? '진행 오류 · 앱을 다시 실행하세요'
    ...
    : balance < 100 ? '잔액 부족 · 메뉴에서 새 시작' : 'P Player · B Banker · T Tie';
  return <>
    <section className="baccarat-table" aria-label="바카라 카드">
      <div className="baccarat-hands">{(['player', 'banker'] as const).map(side => <div className="hand" key={side} aria-label={side === 'player' ? 'Player 패' : 'Banker 패'}>
        <small>{side === 'player' ? 'Player' : 'Banker'} · {state[side].total ?? '–'}</small>
        <div>{state[side].cards.length ? state[side].cards.map(card => <span className="card" key={card.cardId}>{card.rank}{suits[card.suit]}</span>) : <span className="card">–</span>}</div>
      </div>)}</div>
    ...
    <section className="bet actions baccarat-actions">
      {saveError ? <button disabled={busy} onClick={() => void onRetry()}>저장 재시도</button> : internalError ? <span>진행 오류</span>
        : state.phase === 'result' ? <>
          <button disabled={busy} onClick={() => balance < 100 ? onMenu() : void runAction({ type: 'nextRound' })}>{balance < 100 ? '메뉴에서 새 시작' : '다음 판'}</button>
