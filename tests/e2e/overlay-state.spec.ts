...
test('S10-01 접힘은 140×30 DIP이고 반복 전환 후에도 펼친 크기를 복원한다', async () => {
  ...
  await launched.page.evaluate(() => window.blackjack.windowCommand('collapse'));
  await expect.poll(() => getWindowBounds(launched.app)).toMatchObject({ width: 140, height: 30 });
  await expect(launched.page.evaluate(() => window.blackjack.resize({ phase: 'start', edge: 'se' }))).rejects.toThrow();

  await launched.page.getByRole('button', { name: '펼치기' }).click();
  await expect.poll(() => getWindowBounds(launched.app)).toEqual(expanded);
  await launched.page.evaluate(() => window.blackjack.windowCommand('expand'));
  await expect.poll(() => getWindowBounds(launched.app)).toEqual(expanded);
});
...
test('S10-03 슬라이더는 20–100%이고 접힘·reload 중 유지, 재실행 시 65%로 초기화한다', async () => {
  ...
  await expect.poll(async () => (await launched.page.evaluate(() => window.blackjack.getOverlayState())).opacityPercent).toBe(20);
  await expect.poll(() => launched.page.evaluate(() => getComputedStyle(document.querySelector('.overlay')!).opacity)).toBe('0.2');
  await slider.focus();
  await slider.press('Home');
  await expect(slider).toHaveValue('20');
  expect(await launched.page.evaluate(() => window.blackjack.getOverlayState())).toMatchObject({ opacityPercent: 20 });
  await expect(launched.page.evaluate(() => window.blackjack.setOpacity(15))).rejects.toThrow();
  await expect(launched.page.evaluate(() => window.blackjack.setOpacity(21))).rejects.toThrow();

  await slider.press('End');
  await expect(slider).toHaveValue('100');
  expect(await launched.page.evaluate(() => window.blackjack.getOverlayState())).toMatchObject({ opacityPercent: 100 });
  await slider.press('Home');
  await expect.poll(() => launched.page.evaluate(() => window.blackjack.getOverlayState())).toMatchObject({ opacityPercent: 20 });
  await launched.page.evaluate(() => window.blackjack.windowCommand('collapse'));
  await expect(launched.page.getByRole('button', { name: '펼치기' })).toBeVisible();
  await launched.page.getByRole('button', { name: '펼치기' }).click();
  await launched.page.getByRole('button', { name: '흰색/검정 전환' }).hover();
  ...
  await expect.poll(async () => (await launched.page.evaluate(() => window.blackjack.getOverlayState())).visibility).toBe('expanded');
  await launched.page.mouse.move(-30, -30);
  await launched.page.getByRole('button', { name: '흰색/검정 전환' }).hover();
  await expect.poll(() => launched.app.windows().length).toBe(2);
  ...
  await expect(launched.page.locator('output')).toHaveText('$2.00');
  expect(await launched.page.evaluate(() => window.blackjack.getOverlayState())).toMatchObject({ visibility: 'expanded', opacityPercent: 65 });
});
...
test('S10-08 조절창 문서는 불투명도만 바꾸고 게임·창 명령은 실행하지 못한다', async () => {
  ...
  await expect.poll(() => launched.app.windows().length).toBe(2);
  const panel = launched.app.windows()[1]!;
  expect(await panel.evaluate(() => window.blackjack.getOverlayState())).toMatchObject({ opacityPercent: 65 });
  await expect(panel.evaluate(() => window.blackjack.getSnapshot())).rejects.toThrow();
  await expect(panel.evaluate(() => window.blackjack.windowCommand('hide'))).rejects.toThrow();
  await panel.evaluate(() => window.blackjack.setOpacity(20));
  expect(await launched.page.evaluate(() => window.blackjack.getOverlayState())).toMatchObject({ opacityPercent: 20 });
});
...
test('S10-09 헤더 접기 버튼은 없고 Alt+백틱 숨기기 등록·복원 경로를 유지한다', async () => {
  ...
  const hiddenState = await launched.page.evaluate(() => window.blackjack.getOverlayState());
  expect(hiddenState.visibility).toBe('hidden');
  await launched.page.evaluate(() => window.blackjack.windowCommand('hide'));
  expect(await launched.page.evaluate(() => window.blackjack.getOverlayState())).toEqual(hiddenState);
...
for (const mode of ['hide', 'collapse'] as const) test(`S10-${mode === 'hide' ? '04' : '05'} ${mode === 'hide' ? '숨긴' : '접힌'} 동안에도 딜러가 계속 진행한다`, async () => {
  ...
  const result = await launched.page.evaluate(async value => {
    const pending = window.blackjack.dispatch({ commandId: crypto.randomUUID(), expectedRevision: value.revision,
      action: { type: 'stand', handId: value.handId } });
    await window.blackjack.windowCommand(value.mode);
    return pending;
  }, { revision: initial.revision, handId, mode });
  expect(result).toMatchObject({ ok: true, state: { phase: 'dealerTurn' } });
  await expect.poll(async () => (await launched.page.evaluate(() => window.blackjack.getSnapshot())).phase).toBe('result');
  const completed = await launched.page.evaluate(() => window.blackjack.getSnapshot());
  ...
  expect(await launched.page.evaluate(() => window.blackjack.getSnapshot())).toEqual(completed);
});
