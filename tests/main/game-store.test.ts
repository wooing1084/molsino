...
describe('GameStore', () => {
  it('owns the state, publishes revisions, and returns cached duplicate results once', () => {
    ...
    const first = store.dispatch(deal);
    const duplicate = store.dispatch(deal);
  ...
  it('rejects stale revisions without changing state', () => {
    ...
    const stale = store.dispatch(command(
  ...
  it('keeps the hole card and shoe private and advances the dealer internally', () => {
    const shoe = fixtureShoe([
      card('10'), card('9', 'C'), card('7', 'D'), card('7', 'H'), card('5', 'S', '2'),
    ]);
    const store = new GameStore(createSession(shoe), environment(), 'test');
    const dealt = store.dispatch(command(
    ...
    const settled = store.dispatch(command(
    ...
    expect(settled).toMatchObject({
      ...
      state: {
        revision: 3,
        phase: 'result',
        dealerHand: { hiddenCardCount: 0, total: 21 },
      },
    });
  });
});
